<template>
  <div class="home" ref="home">
    <div class="home-flex">
      <div class="header-content">
        <div ref="specialConfiguration">
          <HeaderInfo
            :stationData="WorkStationData"
            :showSpecialConfiguration="showSpecialConfiguration"
            :selectStationData="selectStationData"
            @SelectStation="openSelectStationDialog"
            @SearchDate="SearchDateFun"
          >
          </HeaderInfo>
        ...
        <div class="top-icon">
          <div
            class=""
            style="float: left; line-height: 40px; font-size: 12px"
            v-if="WorkStationData.OrderStatus"
          >
            <span style="padding-right: 30px"
              >单据状态：{{
                WorkStationData.OrderStatus == 8 ||
                WorkStationData.OrderStatus == 14
                  ? "已关闭"
                  : "检验中"
              }}
            </span>
          ...
          <i
            :class="
              !showSpecialConfiguration
                ...
                : 'el-icon-arrow-up'
            "
            @click="showSpecialConfigurationFun"
          ></i>
          <span class="number" v-if="dataList.length"
            >{{ dataList.length - notFinished }}/{{ dataList.length }}</span
          >
        </div>
      </div>
          ...
          <div class="home-content" ref="homeContent" v-if="contentHeightNumer">
            <div
              ...
              :style="{
                height:
                  contentHeightNumer -
                  (![8, 14].includes(WorkStationData.OrderStatus) ? 66 : 0) +
                  'px',
                position: 'relative',
              }"
            ...
            >
              <TableContent
                ...
                "
                :showSpecialConfiguration="showSpecialConfiguration"
                :tableIndex="dataListIndex"
                @updateTableIndex="updateTableIndexFun"
              >
              </TableContent>
              <div
                style="padding: 15px 0px 10px"
                v-if="![8, 14].includes(WorkStationData.OrderStatus)"
              >
            ...
            <div
              class="from-content"
              :style="{ height: contentHeightNumer + 'px' }"
              v-show="!showTableContent"
            >
              <FromQualitative
                @backTable="backTable"
                ...
                :showNext="dataListIndex + 1 != dataList.length"
                @clickNext="clickNextFun"
                :dataObj="dataList[dataListIndex]"
                :WorkStationData="WorkStationData"
                @PartialModificationData="PartialModificationDataFun"
                @IsLastOne="IsLastOneFun"
              ></FromQualitative>
              <FromQuantitative
                @backTable="backTable"
                @updateList="updateList"
                v-if="dataList[dataListIndex].Characteristicattr == 'DL'"
                @clickNext="clickNextFun"
                :dataObj="dataList[dataListIndex]"
                :WorkStationData="WorkStationData"
                :showNext="dataListIndex + 1 != dataList.length"
    ...
    <SelectStation
      ...
      @updateStation="updateStationFun"
    >
    </SelectStation>
  </div>
</template>
...
export default {
  ...
  mounted() {
    if (localStorage.getItem("formLabelAlignStation")) {
      this.selectStationData = JSON.parse(
        localStorage.getItem("formLabelAlignStation"),
      );
    }
    setTimeout(() => {
  ...
  methods: {
    setHeight() {
      setTimeout(() => {
        this.contentHeightNumer =
          this.$refs.home.offsetHeight -
    ...
    updateList() {
      this.$confirm("更新后将清空已检验数据，是否确认更新？", "提示", {
        ...
        .then(() => {
          this.getData({
            Update: true,
            WipOrderNo: this.WorkStationData.WipOrderNo,
            WorkStation: this.selectStationData.wrokStation,
          });
        })
        .catch(() => {});
    ...
    sortedList(list) {
      ...
      const zeroItems = sorted.splice(
        sorted.findIndex((item) => item.SequenceNo === 0),
        1,
      );
