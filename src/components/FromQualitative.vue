...
<!-- 定性 -->
<template>
    <div class="from-name from-qualitative" ref="fromQualitative" style="{height:heightNumer+'px'}">
        <template v-if="fileInput.length">
            <input type="file" v-for="item in fileInput" :key="item.id" :id="item.id" @change="handleFileChange"
                accept="image/*" multiple="multiple" />
        </template>
        <div ref='formName'>


            <el-form style="min-height: 450px;" :rules="formLabelAlignRules" ref='ruleForm'
                :label-position="labelPosition" label-width="80px" :model="formLabelAlign">
                <el-form-item label="项目代码" prop="Characteristic" v-show="false">
                    <el-input readonly v-model="formLabelAlign.Characteristic"></el-input>
                </el-form-item>
                <el-form-item class="form-focus" label="项目描述" prop="CharacteristicText">
                    <!-- <div class="textarea-box">{{ formLabelAlign.CharacteristicText }}</div> -->
                    <el-input type="textarea" disabled :autosize="{ minRows: 7, maxRows: 8 }"
                        v-model="formLabelAlign.CharacteristicText">
                    </el-input>
                </el-form-item>
                <el-form-item :label="formLabelAlign.EquipCodeTypeName" prop="ScanEquipCode" v-if="formLabelAlign.EquipCodeType">
                    <el-input readonly type="textarea" v-model="formLabelAlign.ScanEquipCode"></el-input>
                </el-form-item>
                <el-form-item class="radioList" label="检验判定" prop="Status">
                    <el-radio-group v-model="formLabelAlign.Status" :disabled='isOrderFleg'>
                        <el-radio :label="14">OK</el-radio>
                        <el-radio :label="8">NG</el-radio>
                    </el-radio-group>
                </el-form-item>


                <el-form-item class="radioList" label="异常触发" prop="CreateType">
                    <!-- <el-radio-group v-model="formLabelAlign.CreateType">
                    <el-radio label="Quality">不合格票触发</el-radio>
                    <el-radio label="Aondon">Aondon触发</el-radio>
                </el-radio-group> -->
                    <el-checkbox-group v-model="formLabelAlign.CreateTypes" :disabled='isOrderFleg'>
                        <el-checkbox label="Quality">不合格票触发</el-checkbox>
                        <el-checkbox label="Andon">Andon触发</el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
                <el-form-item class="imagesList" label="图片" prop="imgfileList"
                    :rules="[{ required: formLabelAlign.Status == 8 ? true : false, message: '请选择图片', trigger: 'change' }]">
                    <div class="imgUrls">（{{ imgfileList.length }}/30）</div>
                    <div class="upload-demo-list">
                        <div v-if="imgfileList.length < 30 && !isOrderFleg" v-loading="imgLoading"
                        ...
                        <template v-if="imgfileList.length">
                            <div class="el-upload el-upload--picture-card el-upload--picture-card-imgs"
                                v-for="item, index in imgfileList" :key="index">
                                <el-image :src="item" :preview-src-list="[item]" class="avatar" />
                        ...
                        :multiple="true" :show-file-list="false" :on-change="httpRequest" :on-exceed="shebeiimgExceed"
                        :limit="10" :auto-upload="false">
                ...
                <!--class="w40"-->
                <el-row class="dynamic-item" :gutter="10" v-for="(item, index) in RemarkList" :key="index" >
                    <el-col :xs="20" :sm="20">
                        <el-form-item :label="'问题'+(index+1)" class="mobile-form-item">
                            <el-input :autosize="{ maxRows: 5 }" ref="inputRef" :disabled='isOrderFleg' type="textarea" class="mobile-textarea"
                                @focus="handleFocus" @blur="handleBlur" v-model="RemarkList[index]"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :xs="4" :sm="4">
                        <i @click="remarkListClick(index)" style="width: 100%; height: 100%; line-height: 40px;  font-size: 24px;" class="action-icon"
                            :class="index == 0 ? 'el-icon-circle-plus-outline' : 'el-icon-remove-outline'"></i>
                    </el-col>
                    <el-col :xs="20" :sm="20">
                        <el-form-item :label="'缺陷'+(index+1)" class="mobile-form-item" >
                             <el-select v-model="DefectCodeList[index]"  filterable   class="mobile-select" :disabled='isOrderFleg'
                              @focus="handleFocus" @blur="handleBlur"  ref="DefectCodeSelect" @hook:mounted="handleClick" @visible-change="handleClick"
                              :rules="[{required: !RemarkList[index] ?true:false,message: '请选择缺陷', trigger: ['blur','change']}]">
                                <el-option
                ...
                <el-form-item label="" prop="Remark" style="padding-bottom: 75px;"
        ...
        <div class="btn-list" style="">
            ...
            <el-button @click="closeClick" v-if="!isOrderFleg">重置</el-button>
            <el-button type="primary" v-if="!isOrderFleg" :loading="loadingTrue" @click="submitForm('next')">保存并继续</el-button>
            <!--
            <el-button v-if="showNext && !isOrderFleg" :loading="loadingTrue" type="primary"
...
// import MomUploader from "./MomUploaderPlus.vue"; // 上传组件
export default {
    name: "FromQualitative",
    components: {
        // MomUploader
    },
    data() {
    ...
    watch: {
        dataObj() {
            ...
            if (this.WorkStationData.OrderStatus == 14 || this.WorkStationData.OrderStatus == 8) {
            ...
            this.formLabelAlign.Status = this.dataObj.Status != 8 ? 14 : 8;
    ...
    methods: {
        ...
        adjustPosition(type) {
            ...
            if (type == 'focus') {
                ...
                setTimeout(() => {
                    this.$refs.fromQualitative.scrollTop = this.$refs.formName.scrollHeight;
                }, 100)
            } else {
        ...
        plusFlies() {
            ...
            try {
                ...
                }, { filter: "image", multiple: true, maximum: 10, system: false });// 最多选择3张图片  
        ...
        // 超出限制提示
        shebeiimgExceed() {
            this.$message.error('设备照片只能上传30张');
        },
        ...
        openConfirm(typeFleg) {
            this.$confirm('是否确认提交并关闭检验单?（关闭后将无法再修改！）', '提示', {
        ...
        submitForm(type) {
            ...
            if(this.formLabelAlign.ScanEquipCode==""&&this.formLabelAlign.EquipCodeType){
            ...
            const filteredArray = this.RemarkList.filter(item => item && item.trim() !== "");
            this.formLabelAlign.Remark = filteredArray.length ? filteredArray.join(";") : '';
            this.formLabelAlign.TestReasonCode = this.DefectCodeList.join(",");
            this.$refs['ruleForm'].validate((valid) => {
                if (valid) {
                    ...
                    this.$emit("IsLastOne", (notDetectedList) => {
                        ...
                        this.formLabelAlign.TestAttribute = this.formLabelAlign.Status == 14 ? "OK" : "NG";
