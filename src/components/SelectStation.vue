      ...
      <el-row>
        <el-col :xs="24" :sm="12">
          <el-form-item label="工厂" prop="facility">
            <el-select
              v-model="formLabelAlign.facility"
              placeholder="请选择工厂"
              @change="facilityChang"
            >
              <el-option
          ...
          <el-form-item label="产线" prop="productionLine">
            <el-select
              v-model="formLabelAlign.productionLine"
              placeholder="请选择产线"
              @change="productionLineChang"
            >
              <el-option
                v-for="item in ProductionLineList"
                :label="item.ProductionLineNo + '-' + item.ProductionLineDesc"
          ...
          <el-form-item label="工位" prop="wrokStation">
            <el-select
              v-model="formLabelAlign.wrokStation"
              placeholder="请选择工位"
            >
              <el-option
                v-for="(item, index) in WrokStationList"
                :label="item.WorkStation + '-' + item.WorkStationDesc"
                :value="item.WorkStation"
                :key="index"
              ></el-option>
            </el-select>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      WrokStationList: [],
      formLabelAlignRules: {
        facility: [{ required: true, message: "请选择工厂" }],
        productionLine: [{ required: true, message: "请选择产线" }],
        wrokStation: [{ required: true, message: "请选择工位" }],
      }, //工位验证信息
    };
  },
  props: {
  ...
  mounted() {
    if (localStorage.getItem("formLabelAlignStation")) {
      ...
      this.WrokStationList = this.ProductionLineList.filter((item) => {
        return item.ProductionLineNo == data.productionLine;
      })[0].Data;
  ...
  methods: {
    ...
    productionLineChang(data) {
      this.WrokStationList = this.ProductionLineList.filter((item) => {
        return item.ProductionLineNo == data;
      })[0].Data;
      this.formLabelAlign.wrokStation = "";
    },
    open() {
