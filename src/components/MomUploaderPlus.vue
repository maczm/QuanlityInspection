<template>
  <div class="mom-upload-panel">
    <div class="upload-img-list" data-flex="dir:left wrap">
      ...
      <div
        v-if="mvalue.length < calcMaxCount && calcShowUpload"
        class="mom-uoload"
        v-bind:class="msg ? 'mom-upload-loading' : ''"
        data-flex="cross:center main:center"
      >
        <van-loading v-if="msg" class="upload-van-loading" size="24px"
          >{{ msg }}...</van-loading
        >
        <template v-else>
